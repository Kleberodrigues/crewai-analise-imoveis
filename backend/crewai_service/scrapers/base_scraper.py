...
Com suporte anti-deteccao (Cloudflare bypass)
"""
...
# Tenta importar stealth, mas funciona sem
try:
...
class BaseLeilaoScraper(ABC):
    ...
    async def iniciar(self, stealth_mode: bool = True) -> None:
        """Inicia o browser Playwright com anti-deteccao"""
        try:
            self.playwright = await async_playwright().start()

            # Configuracoes para evitar deteccao
            ...
            # Cria contexto com configuracoes realistas
            ...
            # Aplica stealth mode para evitar deteccao
            if stealth_mode and STEALTH_AVAILABLE:
                stealth = Stealth()
                await stealth.apply_stealth_async(self.page)
                logger.info(f"[{self.FONTE_NOME}] Stealth mode aplicado")
            ...
            logger.info(f"[{self.FONTE_NOME}] Browser iniciado com stealth mode")
